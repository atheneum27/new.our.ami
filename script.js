...
function createGrid() {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        rowHeader.textContent = rowHeaderNames[i] || `Row ${i + 1}`;
...
function updateCellContent(cell, row, col) {
    ...
    if (cellData && cellData.image) {
        ...
        img.onerror = () => console.error('Failed to load image for row', row);
        cell.appendChild(img);
    }
}

// Load sheet from Firebase
function loadSheet() {
    database.ref('spreadsheetData').once('value').then((snapshot) => {
        data = snapshot.val() || Array(rows).fill().map(() => Array(cols).fill({ text: '', image: '' }));
...
function clearSheet() {
    if (confirm('Are you sure you want to clear all signatures?')) {
        data = Array(rows).fill().map(() => Array(cols).fill({ text: '', image: '' }));
        database.ref('spreadsheetData').set(data).then(() => {
            updateGrid();
            alert('Sheet cleared!');
        }).catch(error => {
            console.error('Error clearing sheet:', error);
            alert('Failed to clear sheet.');
        });
    }
}
...
function updateGrid() {
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            ...
            if (cell) updateCellContent(cell, i, j);
        }
        const rowHeader = sheet.rows[i]?.cells[0];
        if (rowHeader) rowHeader.textContent = rowHeaderNames[i] || `Row ${i + 1}`;
    }
    checkFullState();
}
